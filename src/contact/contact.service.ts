...
export class ContactService {
  ...
  async sendDemoRequest(data: ContactDto) {

    // API key and recipient loaded from environment variables
    // Never hardcoded — stored safely in Vercel dashboard
    const SENDGRID_API_KEY = process.env.SENDGRID_API_KEY;
    const TO_EMAIL         = process.env.TO_EMAIL;         // where you receive requests
    const FROM_EMAIL       = process.env.FROM_EMAIL;       // your verified SendGrid sender

    if (!SENDGRID_API_KEY || !TO_EMAIL || !FROM_EMAIL) {
      throw new InternalServerErrorException(
        'Email configuration missing. Check Vercel environment variables.'
      );
    }
    ...
    const emailPayload = {
      personalizations: [
        {
          to: [{ email: TO_EMAIL }],
          subject: `New Demo Request — ${data.name} (${data.industry})`,
        },
      ],
      from: {
        email: FROM_EMAIL,
              ...
              <div style="padding: 2rem; background: #f4f7fb;">
                ...
                <table style="width: 100%; border-collapse: collapse;">
                  <tr>
                    ...
                    <td style="padding: 0.8rem; background: #fff; border-bottom: 1px solid #e0e0e0; color: #333;">${data.name}</td>
                  </tr>
                  <tr>
                    ...
                    <td style="padding: 0.8rem; background: #fff; border-bottom: 1px solid #e0e0e0;">
                      <a href="mailto:${data.email}" style="color: #0E9B8A;">${data.email}</a>
                    </td>
                  </tr>
                  <tr>
                    <td style="padding: 0.8rem; background: #fff; border-bottom: 1px solid #e0e0e0; font-weight: bold; color: #0F3260;">Phone</td>
                    <td style="padding: 0.8rem; background: #fff; border-bottom: 1px solid #e0e0e0; color: #333;">${data.phone || 'Not provided'}</td>
                  </tr>
                  <tr>
                    <td style="padding: 0.8rem; background: #fff; border-bottom: 1px solid #e0e0e0; font-weight: bold; color: #0F3260;">Industry</td>
                    <td style="padding: 0.8rem; background: #fff; border-bottom: 1px solid #e0e0e0;">
                      <span style="background: #E8F0FB; color: #0F3260; padding: 0.2rem 0.8rem; border-radius: 20px; font-size: 0.85rem;">${data.industry || 'Not specified'}</span>
                    </td>
                  </tr>
                  <tr>
                    <td style="padding: 0.8rem; background: #fff; font-weight: bold; color: #0F3260; vertical-align: top;">Message</td>
                    <td style="padding: 0.8rem; background: #fff; color: #333;">${data.message || 'No message provided'}</td>
                  </tr>
                </table>
