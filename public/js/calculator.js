...
// Base material quantities per building type (for 1 floor)
const MATERIAL_BASE = {
...
// Material display config: icon, label, unit
...
/**
 * Calculate and render required materials
 * Called when building type or floor count changes
 */
function calcMaterials() {
  const typeEl  = document.getElementById('build-type');
  const floorEl = document.getElementById('floors');

  // Guard: elements might not be in the DOM yet
  ...
  // Add roofing timber (always floor-dependent)
  items.push({
    icon: '🪵',
    name: 'Roofing Timber',
    unit: 'pieces',
    qty:  Math.round(40 * floors),
  });
